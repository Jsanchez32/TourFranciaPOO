const url = "http://localhost:4000/equipo"

export const mostrar = async ()=>{
    try {
        const equipos = await fetch(`${url}/all`);
        const response = equipos.json();
        return response
    } catch (error) {
        console.log(error);
    }
...
export const selectOne = async(id)=>{
    try {
        const equipo = await fetch(`${url}/one/${id}`)
        const response = equipo.json();
        return response;
    } catch (error) {
        console.log(error);
    }
