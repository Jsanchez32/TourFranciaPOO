...
async function mostrarDatos(){
    ...
    const ciclistas = await mostrar();
    ciclistas.forEach((ciclista)=>{
        const {nombre,edad,_id,nacionalidad,equipo}=ciclista
        console.log(edad);
        contenedor.innerHTML+=`
            ...
            <div class="content">
                ...
                <div class="front-content">
                    <small class="badge">Nacionalidad: ${nacionalidad}</small>
                    <small class="badge">Equipo: ${equipo}</small>
...
async function mostrarOne(e){
    ...
    const ciclistaId = e.target.getAttribute('id');
    const datos = await selectOne(ciclistaId);

        const nombreUpdate =document.querySelector('#nombreUpdate');
        const edadUpdate = document.querySelector('#edadUpdate');
        const nacionalidadUpdate = document.querySelector('#nacionalidadUpdate');
        const equipoUpdate = document.querySelector('#equipoUpdate');
        const id = document.querySelector('#idUpdate');

        nombreUpdate.value=datos.nombre;
        edadUpdate.value=datos.edad;
        nacionalidadUpdate.value=datos.nacionalidad;
        equipoUpdate.value=datos.equipo;
        id.value=ciclistaId;
}
...
async function actualizarDato(e){
    ...
    const nacionalidad = document.querySelector('#nacionalidadUpdate').value;
